...
# ---------- 마스터 통계 집계 함수 (14개 과목 전체 통계 + 구체적인 최다 오답 문제 디테일 분석 포함) ----------
def get_master_statistics():
    ...
    try:
        ...
        # 각 과목별로 가장 많이 틀린 구체적인 문제 디테일 분석 (과목별 최다 오답 문제)
...
def view_dashboard():
    ...
    # =====================================================================
    # [마스터 전용 통계 분석 섹션] (14개 과목 전체 통계 + 구체적인 오답 문항 분석 포함)
    # =====================================================================
    if user["email"] == "trustkimjs@police.go.kr":
        ...
        if stats:
            ...
            # 2포인트: 구체적인 최다 오답 문제 디테일 분석 (어떤 문제를 가장 많이 틀렸는지 지문과 함께 표시)
            st.markdown('<p style="font-weight:700;font-size:1rem;color:#e63946;">⚠️ 수험생들이 가장 많이 틀린 구체적인 문항 분석 (TOP 10)</p>', unsafe_allow_html=True)
            if stats["detailed_worst_questions"]:
                for idx, wq in enumerate(stats["detailed_worst_questions"], 1):
                    ...
                    stem_preview = (stem_text[:60] + '...') if len(stem_text) > 60 else stem_text
                    st.markdown(
                        f"""
