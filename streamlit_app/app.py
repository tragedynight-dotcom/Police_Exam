...
# =====================================================================
# [안전한 백엔드 패치] 스트림릿 재실행 시 NameError 차단 (학습모드 시간초과 무력화)
# =====================================================================
...
def restore_user_from_url():
    ...
    # [핵심 패치 1] URL 링크 공유 시 계정 유출을 막기 위해, 읽자마자 주소창에서 토큰을 삭제합니다!
    if "auth" in st.query_params:
        del st.query_params["auth"]
...
def login_success(user: dict, view: str = "dashboard", **kwargs):
    ...
    # [핵심 패치 2] 로그인 시 더 이상 주소창에 토큰을 남기지 않습니다. 오직 부모창(깃허브) 기억장치로만 몰래 보냅니다.
    components.html(f"""
        <script>
        try {{
...
def app_shell_css():
    st.html(
        ...
        <style>
          /* ================================================== */
          /* [핵심 패치 3] 다크모드 대응: 배경을 흰색으로 고정했으므로, 기본 글자색도 어두운 색으로 강력 고정! */
          /* ================================================== */
          ...
          /* 위 요소들 중 특정 디자인 버튼이나 배너의 흰 글씨는 보호 */
