...
# =====================================================================
# [안전한 백엔드 패치] 스트림릿 재실행(Rerun) 시 발생하는 NameError 완벽 차단
# =====================================================================
if not getattr(_lib_exam, "_timer_patched", False):
    _orig_is_time_expired = _lib_exam.is_time_expired
    _orig_attempt_ends_at = _lib_exam.attempt_ends_at
    
    # 변수가 증발하지 않도록 orig 파라미터에 영구적으로 박제(Binding)합니다.
    def _safe_is_time_expired(attempt, orig=_orig_is_time_expired):
        if attempt and attempt.get("revealMode") == "immediate":
            return False
        return orig(attempt)
        
    def _safe_attempt_ends_at(attempt, orig=_orig_attempt_ends_at):
        if attempt and attempt.get("revealMode") == "immediate":
...
def app_shell_css():
    ...
    # --------- [핵심 패치] 해킹 에러 없이 스트림릿 화면 내에서만 사운드 및 1줄 고정 적용 ---------
    components.html(
        """
        <script>
        (function() {
            ...
            let win = window;
            try {
                // 부모 창 접근 시 보안에러(SecurityError)가 나면 무시하고 스트림릿 창만 사용
                if (window.parent && window.parent.document) {
                    doc = window.parent.document;
                    win = window.parent;
                }
            } catch(e) {} // 에러 발생 시 원래의 스트림릿 창(win, doc)을 그대로 사용함!
...
def view_exam():
    ...
    # [수정 완료] 마커(.exam-nav-side-mark)를 nav_l 안에 안전하게 배치
